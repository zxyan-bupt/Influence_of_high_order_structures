...
for n_theta=-0.5:0.01:1
    i_theta=i_theta+1;
    theta(i_theta)=n_theta;
    phi6(i_theta)=-theta(i_theta);
    for i_k=1:k_max
        sigma=(A+B/i_k)*lambda_delta/lambda1;
        phi6(i_theta)=phi6(i_theta)+P1(i_k+1)/k_bar* (i_k*i_k*lambda1*theta(i_theta)*(1+sigma*theta(i_theta))) / (1+ i_k*lambda1*theta(i_theta)*(1+sigma*theta(i_theta)) );
    end
end
...
for n_theta=-0.5:0.01:1
    i_theta=i_theta+1;
    theta(i_theta)=n_theta;
    phi7(i_theta)=-theta(i_theta);
    for i_k=1:k_max
        sigma=(A+B/i_k)*lambda_delta/lambda1;
        phi7(i_theta)=phi7(i_theta)+P1(i_k+1)/k_bar* (i_k*i_k*lambda1*theta(i_theta)*(1+sigma*theta(i_theta))) / (1+ i_k*lambda1*theta(i_theta)*(1+sigma*theta(i_theta)) );
    end
end
...
for n_theta=-0.5:0.01:1
    i_theta=i_theta+1;
    theta(i_theta)=n_theta;
    phi8(i_theta)=-theta(i_theta);
    for i_k=1:k_max
        sigma=(A+B/i_k)*lambda_delta/lambda1;
        phi8(i_theta)=phi8(i_theta)+P1(i_k+1)/k_bar* (i_k*i_k*lambda1*theta(i_theta)*(1+sigma*theta(i_theta))) / (1+ i_k*lambda1*theta(i_theta)*(1+sigma*theta(i_theta)) );
    end
end
...
for n_theta=-0.5:0.01:1
    i_theta=i_theta+1;
    theta(i_theta)=n_theta;
    phi9(i_theta)=-theta(i_theta);
    for i_k=1:k_max
        sigma=(A+B/i_k)*lambda_delta/lambda1;
        phi9(i_theta)=phi9(i_theta)+P1(i_k+1)/k_bar* (i_k*i_k*lambda1*theta(i_theta)*(1+sigma*theta(i_theta))) / (1+ i_k*lambda1*theta(i_theta)*(1+sigma*theta(i_theta)) );
    end
end
...
subplot(1, 2, 1);
hold on;
plot(theta,phi7,'r','LineWidth',2);
plot(theta,phi8,'g','LineWidth',2);
plot(theta,phi9,'b','LineWidth',2);
% 绘制 y=0 直线（水平线）
plot([-5 5], [0 0],'k--','LineWidth',2); % 蓝色 y=0 直线
% 绘制 x=0 直线（垂直线）
plot([0 0], [-5 5],'k--','LineWidth',2); % 红色 x=0 直线
box on;
axis([-0.2 0.6 -0.1 0.05]);
set(gca, 'FontName', 'Cambria Math'); % 设置字体
set(gca, 'FontSize', 24); % 设置字体大小
set(gca, 'LineWidth', 2); % 设置边框大小
title('(a) \lambda_{\Delta}=0');
...
legend('\lambda=0.045','\lambda=0.0471','\lambda=0.06','Location','eastoutside','Box','off','FontSize',16);

subplot(1, 2, 2);
hold on;
plot(theta,phi1,'r','LineWidth',2);
plot(theta,phi2,'g','LineWidth',2);
plot(theta,phi3,'b','LineWidth',2);
plot(theta,phi4,'k','LineWidth',2);
plot(theta,phi5,'m','LineWidth',2);
% 绘制 y=0 直线（水平线）
plot([-5 5], [0 0],'k--','LineWidth',2); % 蓝色 y=0 直线
% 绘制 x=0 直线（垂直线）
plot([0 0], [-5 5],'k--','LineWidth',2); % 红色 x=0 直线
...
set(gca, 'FontSize', 24); % 设置字体大小
set(gca, 'LineWidth', 2); % 设置边框大小
title('(b) \lambda_{\Delta}=0.5');
xlabel('\Theta(\infty)'), ylabel('\Phi[\Theta(\infty)]-\Theta(\infty)')
xticks([-0.2 0 0.2 0.4 0.6]); % 设置 x 轴刻度位置
yticks([-0.1 -0.05 0 0.05]); % 设置 y 轴刻度位置
legend('\lambda=0.03','\lambda=0.0434','\lambda=0.045','\lambda=0.0471','\lambda=0.06','Location','eastoutside','Box','off','FontSize',16);

% hold on;
% plot(theta,phi6,'k','LineWidth',2);
% % 绘制 y=0 直线（水平线）
% plot([-5 5], [0 0],'k--','LineWidth',2); % 蓝色 y=0 直线
% % 绘制 x=0 直线（垂直线）
% plot([0 0], [-5 5],'k--','LineWidth',2); % 红色 x=0 直线
